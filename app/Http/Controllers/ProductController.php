...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $products = Product::ByItemCode($request->input('item_code'))
        ->ByTitle($request->input('title'))
        ->ByPrice($request->input('min'),$request->input('max'))
        ->paginate(10);
    ...
    {
        abort_if(Gate::denies('product_access'), Response::HTTP_FORBIDDEN, '403 Forbbiden');
        $config = Config::first();        
    ...
    {       
        ...
        //$tags = Product::where('id',$product->id)->with('tags')->get();
