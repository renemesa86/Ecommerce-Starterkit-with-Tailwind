<x-app-layout>
    <x-slot name="header">
        <x-header title="Editar subcategoría"></x-header>
    </x-slot>
    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('subcategorias.update', $subcategory->id) }}" >
                    @csrf
                    @method('put')
                ...
                <div class="px-4 py-2 bg-white sm:p-2">
                    ...
                    <select name="category" id="category"
                        ...
                        >
                        @foreach($categories as $id => $tag)
                        <option value="{{ $id }}" {{ $subcategory->parent_id == $id ? ' selected' : '' }}>
                            {{ $tag }} 
                        </option>
                        @endforeach
                    </select>
